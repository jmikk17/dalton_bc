...
\begin{list}{}{}
\item Integral: $-\left<\chi_{\mu}\left|{\bf r}_{O}\times\bm\nabla\right|\chi_{\nu}\right>$
...
\item[\Key{GZZEFG}] Gradient of the electronic contribution to the $zz$ cartesian electric field integrals at nuclei. %Equals $\left<\chi_{\mu}\left|\nabla q_{zz}(\textbf{R}_K)\right|\chi_{\nu}\right>$ in a.u..

\begin{list}{}{}
%\item Integral:
%$\left<\chi_{\mu}\left|\nabla q_{zz}(\textbf{R}_K)\right|\chi_{\nu}\right>$ where $K$ is the nucleus of interest.
\item Integral: $\frac{1}{3}\left<\chi_{\mu}\left|\frac{\partial}{\partial i}\left(\frac{3z_K^2}{r_K^5}-\frac{1}{r_K^3}\right)\right|\chi_{\nu}\right>$ where $K$ is the nucleus of interest. \\
\item Property labels: \verb|GEFG abc |, where \verb|abc| depends on the nucleus of interest and the gradient component. \\
%E.g.: $GEFG\;\;002 \equiv \left<\chi_{\mu}\left|\frac{\partial q_{zz}(\textbf{R}_1)}{\partial y}\right|\chi_{\nu}\right>$ and $GEFG\;\;006 \equiv \left<\chi_{\mu}\left|\frac{\partial q_{zz}(\textbf{R}_2)}{\partial z}\right|\chi_{\nu}\right>$. \\
E.g.: $GEFG\;\;002 \equiv \frac{1}{3}\left<\chi_{\mu}\left|\frac{\partial}{\partial y}\left(\frac{3z_1^2}{r_1^5}-\frac{1}{r_1^3}\right)\right|\chi_{\nu}\right>$ and $GEFG\;\;006 \equiv \frac{1}{3}\left<\chi_{\mu}\left|\frac{\partial}{\partial z}\left(\frac{3z_2^2}{r_2^5}-\frac{1}{r_2^3}\right)\right|\chi_{\nu}\right>$.
\end{list}
...
\item[\Key{LAPEFG}] Laplacian of the electronic contribution to the $xx$, $yy$ and $zz$ cartesian electric field integrals at nuclei.

\begin{list}{}{}
%\item Integral: $\left<\chi_{\mu}\left|\nabla^2 q_{ii}(\textbf{R}_K)\right|\chi_{\nu}\right>$ where $K$ is the nucleus of interest.
\item Integral: $\frac{1}{3}\left<\chi_{\mu}\left|\frac{\partial^2}{\partial i^2}\left(\frac{3j_K^2}{r_K^5}-\frac{1}{r_K^3}\right)\right|\chi_{\nu}\right>$ where $K$ is the nucleus of interest and $i,j= \{x,y,z\}$. \\
\item Property labels: \verb|LEFG abc|, where \verb|abc| depends on the nucleus of interest and the laplacian component. There are nine integrals for each nucleus. \\
E.g.:
\begin{eqnarray*}
%LAPEFG\;\;010 &\equiv& \left<\chi_{\mu}\left| \frac{\partial^2 q_{xx}(\textbf{R}_2)}{\partial x^2}\right|\chi_{\nu}\right>   \\
LAPEFG\;\;010 &\equiv& \frac{1}{3}\left<\chi_{\mu}\left|\frac{\partial^2}{\partial x^2}\left(\frac{3x_2^2}{r_2^5}-\frac{1}{r_2^3}\right)\right|\chi_{\nu}\right>   \\
%LAPEFG\;\;012 &\equiv& \left<\chi_{\mu}\left| \frac{\partial^2 q_{xx}(\textbf{R}_2)}{\partial z^2}\right|\chi_{\nu}\right> \\
LAPEFG\;\;012 &\equiv& \frac{1}{3}\left<\chi_{\mu}\left|\frac{\partial^2}{\partial z^2}\left(\frac{3x_2^2}{r_2^5}-\frac{1}{r_2^3}\right)\right|\chi_{\nu}\right>   \\
%LAPEFG\;\;016 &\equiv& \left<\chi_{\mu}\left| \frac{\partial^2 q_{zz}(\textbf{R}_2)}{\partial x^2}\right|\chi_{\nu}\right> \\
LAPEFG\;\;016 &\equiv& \frac{1}{3}\left<\chi_{\mu}\left|\frac{\partial^2}{\partial x^2}\left(\frac{3z_2^2}{r_2^5}-\frac{1}{r_2^3}\right)\right|\chi_{\nu}\right>   \\
%LAPEFG\;\;018 &\equiv& \left<\chi_{\mu}\left| \frac{\partial^2 q_{zz}(\textbf{R}_2)}{\partial z^2}\right|\chi_{\nu}\right> \\
LAPEFG\;\;018 &\equiv& \frac{1}{3}\left<\chi_{\mu}\left|\frac{\partial^2}{\partial z^2}\left(\frac{3z_2^2}{r_2^5}-\frac{1}{r_2^3}\right)\right|\chi_{\nu}\right>
\end{eqnarray*}
...
\item[\Key{LR-SHI}] All needed integrals to compute relativisctic corrections 
to nuclear magnetic shielding will be done. 
\item[\Key{LR-EFG}] All needed integrals to compute relativisctic corrections 
to electric field gradients will be done.
%\begin{list}{}{}
%\item Integral:
%\end{list}
...
\item[\Key{NUCPOT}] Calculate the potential energy from each individual nucleus.
Currently this keyword can only be used in calculations not employing
symmetry.\index{potential energy!individual nuclei}
...
atom's label must be unique.
Note that the atom labels in .mol input are \em{not} used to identify the atom type,
you can therefore use anything you like, e.g. 'aa', 'ab', 'ac', ...
\end{list}
...
\begin{list}{}{}
\item Integral:
$\left<\chi_{\mu}\left|\frac{{\bf r}_K\times \bm\nabla}{r_{K}^{3}}\right|\chi_{\nu}\right>$
where $K$ is the nucleus of interest.
...
\item[\Key{ERF   }] Default: standard two-electron repulsion integrals.\newline
\verb|READ (LUCMD,*) MU_VALUE| \\
Here \verb|MU_VALUE| is the value of $\mu$ used in the separation of two-electron repulsion integrals in a short-range part and a long-range part using the error function:
\begin{eqnarray*}
Vee &=& Vlr+Vsr \\
Vlr &=& \mathrm{erf}(\mu r_{12})/(r_{12}) \\
Vsr &=& [1 -\mathrm{erf}(\mu r_{12})]/(r_{12}) 
\end{eqnarray*}
This option is used to specify the long-range and the short-range two-electron repulsion integrals
in HF-srDFT, MP2-srDFT, CI-srDFT, MC-srDFT calculations.
...
parallel calculations. The treshold for excluded integrals will be $10^{\rm -IFTHRS}$.
The default is that this value will change dynamically as the wave function converges more and more tightly
...
under \Key{PRINT} above. Only for debugging purposes.
...
to disk. The default is defined in the source file "lbuf\_aotwoint.h".
Longer buffer lengths may give more efficient I/O.
