...
\item{\Key{ATMMOM}}\\
\verb|READ (LUCMD,*) IORATM|\\
Add atomic moment operators using MBIS (Minimum Basis Iterative Stockholder) population analysis to both the $A$ and $B$ operator lists.
The integer \verb|IORATM| sets the multipole order included:
\begin{itemize}
  \item \verb|IORATM = 0|: monopole only
  \item \verb|IORATM = 1|: monopole and dipole 
\end{itemize}
Requires \verb|NoSymmetry| in the molecule input.
\index{atom-atom polarizability}\index{MBIS population analysis}
...
When using triplet operators the \Key{TRIPLET} keyword 
must be set.
...
\item{\Key{SELATM}}\\
\verb|READ (LUCMD,*) NATM|\\
\verb|READ (LUCMD,*) (ATMIND(I), I=1, NATM)|\\
Restrict atomic moment calculations to a subset of \verb|NATM| atoms given by their indices. Must be combined with \Key{ATMMOM}.
\index{atom-atom polarizability}
...
The \Key{TRIPLET} keyword will also be set by this option.
...
calculation which did not converge (you cannot e.g. restart a converged
calculation with a lower convergence threshold, the files necessary for
RESTPP are deleted by DALTON after convergence).
The files RSPRST.E2C, RSP\_BVEC, RSP\_SVEC, and RSP\_DIAG
must be provided for restart, they are by default on the tar.gz file
This can only be used if the first roots requested
has the same symmetry as the trial vectors on file.  \\
If the wave function is MCSCF type, the converged wave function must
be reestablished with .STARTOLDCI and .MOSTART NEWORB options.
...
\item{\Key{ATMMOM}}\\
\verb|READ (LUCMD,*) IORATM|\\
Calculates Cauchy moments and the atom-atom polarizabilities at the default/specified grid points.
The integer \verb|IORATM| controls the multipole order:
\begin{itemize}
  \item \verb|IORATM = 0|: monopole only
  \item \verb|IORATM = 1|: monopole and dipole 
\end{itemize}
...
\item{\Key{PRINT}}     \\
\verb|READ (LUCMD,*),IPRESR |\\
Specify the print level for the HFC routines.
