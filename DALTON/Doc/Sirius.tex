...
\item[\Key{MP2special}]
Special option for computationally cheaper creation of MP2 initial guess orbitals for MCSCF or MC--srDFT than the standard MP2 and MP2srDFT options.
Selects 1) the cheaper SOS-MP2; 2) neglect of two-electron MO integrals having absolute valute less than 1.e-7;
and 3) neglect of the second-order wave function needed for the occupied-virtual block of the MP2 density matrix,
avoiding the need for MO integrals with three general indices.
The relatively small change in the initial guess orbitals compared to full MP2 has not been found to have any significance for the following MCSCF/MC--srDFT calculation.
...
\item[\Key{THR2EL}]
  Default = general threshold\\
  \kw{READ (LUINP,*) THRP\_MP2} \\
  Threshold for neglect of MO 2-electron integrals in MP2 module.
...
\item[\Key{MP2 SCALED}] \ \\
  ...
  Default is no scaling ($p_S = 1$, $p_T = 1$).
...
AO integrals\index{two-electron integral} to molecular orbital
...
\item[\Key{P THR}] \ \\
  \kw{READ (LUINP,*) THRP} \\
  Neglect MO 2-electron integrals with abasolute value lesss than THRP.
